...
% Cubo
cubo=[40+0i 40+190i];
silueta_cubo=[cubo(1), cubo(2), 180/190*exp(i*pi/2)*(cubo(2)-cubo(1))+cubo(2), 180/190*exp(i*pi/2)*(cubo(2)-cubo(1))+cubo(1), cubo(1)];
        ...
        for c1x=cuadrante1(1)-trapez*100/375*(c1y-375):1/n*(cuadrante1(2)-cuadrante1(1)+trapez*100/375*(c1y-375)):cuadrante1(2)
            ...
            for c2y=cuadrante2(3):1/n*(cuadrante2(4)-cuadrante2(3)):cuadrante2(4)
                for c2x=cuadrante2(1)-trapez*100/375*(c2y-375):1/n*(cuadrante2(2)-cuadrante2(1)+trapez*100/375*(c2y-375)):cuadrante2(2)
                    ...
                    while ((imag(cubo(iter,2*k-1))<(0-375)/(100-0)*real(cubo(iter,2*k-1))+375 ...
                            || (imag(cubo(iter,2*k-1))>375) ...
                            && imag(cubo(iter,2*k-1))>=0 ...
                            && real(cubo(iter,2*k-1))<400)) ...
                            && (imag(cubo(iter,2*k))<(0-375)/(100-0)*real(cubo(iter,2*k))+375 ...
                            || imag(cubo(iter,2*k))>375) ...
                            && imag(cubo(iter,2*k))>=0 ...
                            && real(cubo(iter,2*k))<400 ...
                            && 180/190*exp(i*pi/2)*(cubo(iter,2*k)-cubo(iter,2*k-1))+cubo(iter,2*k)<=400 ...
                            && bloqueo==0 ...
                        ...
                        if cosAlpha<=1 && cosAlpha>=-1
                            ...
                            silueta_cubo(iter,5*k-4:5*k)=[cubo(iter,2*k-1),...
                                cubo(iter,2*k),...
                                180/190*exp(i*pi/2)*(cubo(iter,2*k)-cubo(iter,2*k-1))+cubo(iter,2*k),...
                                180/190*exp(i*pi/2)*(cubo(iter,2*k)-cubo(iter,2*k-1))+cubo(iter,2*k-1),...
                                cubo(iter,2*k-1)];
                            ...
                            if real(cubo(iter,2*k-1))>=133 ...
                                    && real(cubo(iter,2*k))>=133 ...
                                    && real(cubo(iter,2*k-1))<=400 ...
                                    && real(cubo(iter,2*k))<=400 ...
                                    && 180/190*exp(i*pi/2)*(cubo(iter,2*k)-cubo(iter,2*k-1))+cubo(iter,2*k)<=400
